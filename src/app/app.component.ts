...
export class AppComponent {
  ...
  products : Array<books> = [
    ...
    insertProduct(e:any){
      ...
      this.total = Math.floor(this.total + e.price.value);
    }
  
    decreaseQuantity(id : any){
      let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === id)
      this.total = Math.floor(this.total - Math.floor(this.cartItems[itemIndex].price))
    ...
    removeItem(id:any){
      let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === id)
      this.total = Math.floor(this.total - this.cartItems[itemIndex].quantity * Math.floor(this.cartItems[itemIndex].price))
      this.cartItems.splice(itemIndex,1)
    }
}
